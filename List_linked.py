...
class List:
    ...
    def insert(self, i, value):
        ...
        if i < -self._count or i > self._count:
            # i is out of range, so prepend or append value as appropriate
            if i < 0:
                self.prepend(value)
            else:
                self.append(value)
    ...
    def remove_many(self, key):
        ...
        """
        # your code here
        v1, v2, _ = self._linear_search(key)
        while v2 is not None:
            if v1 is not None:
                v1._next = v2._next
            if v2 == self._front:
                self._front = v2._next
            if v2 == self._rear:
                self._rear = v1
            self._count -= 1
            v1, v2, _ = self._linear_search(key)
    ...
    def __eq__(self, rs):
        """
        -------------------------------------------------------
        Compares this list against a comparable list or sets.
        Items and order must match.
        Use: l == rs
        -------------------------------------------------------
        Parameters:
            rs - another list or set (list or set)
        Returns:
            result - True if this list contains the same elements as rs
    ...
    def is_identical_r(self, other):
        ...
        if self._front is None:
            ...
            if self._front._value != other._front._value:
                ...
                before_front = other._front
                before_front_self = self._front
                other._front = other._front._next
                self._front = self._front._next
                identical = self.is_identical_r(other)
                other._front = before_front  
                self._front = before_front_self
    ...
    def intersection_r(self, source1, source2):
        ...
        if source1._front is not None and source2._front is not None:
            i = 0
            self._intersection_r_aux(i, source1._front, source1, source2)
    ...
    def union_r(self, source1, source2):
        ...
        if source1._front is not None:
            index = 0
            search = 0
            self._union_r_aux(search, index, source1._front, source1, source2)
        else:
            if source2._front is not None:
                index = 0
                search = 1
                self._union_r_aux(search, index, source2._front, source1, source2)
        return
