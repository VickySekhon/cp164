...
class Hash_Set:
    ...
    def insert(self, value):
        ...
        temp = hash(value)
        placement = temp%self._capacity
        if self._count == 0:
            self._table[placement].append(value)
            inserted = True 
        else:
            if self._table[placement].is_empty():
                self._table[placement].append(value)
                inserted = True 
            else:
                i = self._table[placement].index(value)
                if i == -1:
                    self._table[placement].append(value)
                    inserted = True 
        if inserted:
    ...
    def remove(self, key):
        ...
        if self._count == 0:
            ...
            if self._table[placement].is_empty():
                ...
                i = self._table[placement].index(key)
                if i == -1:
                    value = None 
                    
                else:
                    value = self._table[placement][i]
                    self._table[placement].pop(i)
                    self._count -= 1
